...
class SimpleLSTM(torch.nn.Module):
    def __init__(self, vocab_size: int, hidden_size: int, embed_size: int):
        ...
        # TODO: 학습해야하는 가중치를 정의.
        # 이제 뭐해요?
        # 1. 일단 학습해야하는 가중치를 다 정의를 하고, 차원을 결정하는 것은 스킵.
        # 2. forward에 가서, X로부터 원하는 출력값을 얻기 위한 로직을 작성하면서, **차원을 트래킹한다**
        # 3. (A, B) * (B, C) = (A, C).
        ...
        self.W = torch.nn.Linear(in_features=hidden_size + embed_size, out_features=hidden_size*4)  # (H+ E, H*4).
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        # H_last_1 = ...
        # 이제 뭐하는가?
        for time in range(X.shape[1]):  # t=0 -> t=L-1
            # X_t는 어떻게 구하느냐?
            X_t = X[:, time]  # (N, L) -> (N, 1)
            ...
            F = torch.sigmoid(G[:, : self.hidden_size])  # 망각 게이트
            I = torch.sigmoid(G[:, self.hidden_size: self.hidden_size*2])  # 입력 게이트
            O = torch.sigmoid(G[:, self.hidden_size*2: self.hidden_size*3])  # 출력 게이트
            H_temp = torch.tanh(G[:, self.hidden_size*3: self.hidden_size*4])  # 임시 단기기억
            # C_t는 어떻게 구하죠?
            # 먼저, 이전 장기기억을 가져와서 뭘했었죠? : 지우개
            # 장기기억에 무엇을 더해준다?: 장기기억화할만한 가치가 있는 단기기억을 ㅈ더해준다.
